...
class FileUpload {
    ...
    /*
    **  @param array saveCallbacks -- array of callback that are called after a file is uploaded
    */
    protected $saveCallbacks=array();
	...
	/**
        ...
        * @param array $allowedFields -- the fields to be uploaded -- default all uploaded files
		*
    ...
    */
    ...
    public function setAllFields(){
        $this->setAllowedFields(array_keys($_FILES));
    }
	...
	*/
	public function saveFile(File $file, $uploadDirectory = null) {
		...
		foreach ($this->getConstraints() as $constraint) {
            if(!$constraint->isValid($file)){
                    
				$this->setUploadDirectory($oldUploadDirectory);
				$file->setUploaded(false);
				$this->callErrorClosure(new UploadError(UploadError::ERR_CONSTRAINT, $constraint->getErrors(), $file, $constraint));
				return false;                    
            }
		}

		// save file
		$filePath = $this->getUploadDirectory() . DIRECTORY_SEPARATOR . basename($file->getOriginalName());
	...
	public function save(\Closure $closure = null) {
		...
		foreach ($this->getFiles() as $file) {
            ...
            if($uploadSuccessful){
                foreach($this->saveCallbacks as $callback){
    ...
    /*
    **  @function addSaveCallback -- add callback function to execute after is file is uploaded
    */
